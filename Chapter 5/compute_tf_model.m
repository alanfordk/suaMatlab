function [T_phi_delta_a,T_chi_phi,T_theta_delta_e,T_h_theta,T_h_Va,...
    T_Va_delta_t,T_Va_theta,T_beta_delta_r, lat, lon]...
    = compute_tf_model(x_trim,u_trim,P)
% x_trim is the trimmed state,
% u_trim is the trimmed input

% Define the transfer functions from chapter 5.  You can use the tf([],[])
% function and specify the numerator and denominator coefficients or you
% can use the s variable obtained by a call to tf('s')
% tf([],[]);
% s = tf('s');
% define transfer functions
Va_trim = sqrt(x_trim(4)^2 + x_trim(5)^2 + x_trim(6)^2);
alpha_trim = atan(x_trim(6)/x_trim(4));
theta_trim = x_trim(8);

% define gamma values
gamma = P.Jx*P.Jz-P.Jxz^2;
gamma_three = P.Jz/gamma;
gamma_four = P.Jxz/gamma;

% define extra coefficients
C_p_p = gamma_three*P.C_ell_p + gamma_four*P.C_n_p;
C_p_delta_a = gamma_three*P.C_ell_delta_a + gamma_four*P.C_n_delta_a;

a_phi1 = -.5*P.rho*Va_trim^2*P.S_wing*P.b*C_p_p*P.b/(2*Va_trim);
a_phi2 = .5*P.rho*Va_trim^2*P.S_wing*P.b*C_p_delta_a;
a_beta1 = -P.rho*Va_trim*P.S_wing/(2*P.mass)*P.C_Y_beta;
a_beta2 = P.rho*Va_trim*P.S_wing/(2*P.mass)*P.C_Y_delta_r;
a_theta1 = -P.rho*Va_trim^2*P.c*P.S_wing/(2*P.Jy)*P.C_m_q*P.c/(2*Va_trim);
a_theta2 = -P.rho*Va_trim^2*P.c*P.S_wing/(2*P.Jy)*P.C_m_alpha;
a_theta3 = P.rho*Va_trim^2*P.c*P.S_wing/(2*P.Jy)*P.C_m_delta_e;
a_V1 = P.rho*Va_trim*P.S_wing/P.mass*(P.C_D_0 + P.C_D_alpha*alpha_trim + ...
    P.C_D_delta_e*u_trim(1)) + P.rho*P.S_prop/P.mass*P.C_prop*Va_trim;
a_V2 = P.rho*P.S_prop/P.mass*P.C_prop*P.k_motor^2*u_trim(2);
a_V3 = P.gravity*cos(theta_trim - alpha_trim);

T_phi_delta_a   = tf(a_phi2,[1,a_phi1,0]);
T_chi_phi       = tf(P.gravity/Va_trim,[1,0]);
T_theta_delta_e = tf(a_theta3,[1,a_theta1,a_theta2]);
T_h_theta       = tf(Va_trim,[1,0]);
T_h_Va          = tf(theta_trim,[1,0]);
T_Va_delta_t    = tf(a_V2,[1,a_V1]);
T_Va_theta      = tf(-a_V3,[1,a_V1]);
T_beta_delta_r  = tf(a_beta2,[1,a_beta1]);

% package up coefficients for future assignments
lat.a_phi1 = a_phi1;
lat.a_phi2 = a_phi2;
lon.a_theta1 = a_theta1;
lon.a_theta2 = a_theta2;
lon.a_theta3 = a_theta3;
lon.a_V1     = a_V1;
lon.a_V2     = a_V2;
lon.a_V3     = a_V3;
lat.a_beta1  = a_beta1;
lat.a_beta2  = a_beta2;
